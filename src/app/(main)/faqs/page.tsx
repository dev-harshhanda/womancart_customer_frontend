...
const items = [{ label: "Home", path: "/" }, { label: "FAQs" }];
      ...
      if (typeof arg === "number" || !isNaN(Number(arg))) {
        ...
        const element =
          arg instanceof Element
            ? arg
            : arg.currentTarget instanceof Element
              ? arg.currentTarget
              : null;
        if (!element) return;
        const findParent = (el: Element) => {
          ...
          while (current) {
            if (current.classList?.contains("faq-item")) return current;
        ...
        if (!parent) return;
        content =
          parent.querySelector(".faq-content") ||
          parent.querySelector("[id^=\"content-\"]");
        icon =
          element.querySelector(".faq-icon") ||
          element.querySelector("[id^=\"icon-\"]");
      }
      if (content) {
        const isExpanded =
